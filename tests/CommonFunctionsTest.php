...
class CommonFunctionsTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testResolveGlues()
    {
        ...
        $this->assertSame(array('devices.device_id'), ResolveGlues('devices', 'device_id'));
        $this->assertSame(array('sensors.device_id'), ResolveGlues('sensors', 'device_id'));

        $expected = array('bill_data.bill_id', 'bill_ports.port_id', 'ports.device_id');
        $this->assertSame($expected, ResolveGlues('bill_data', 'device_id'));
